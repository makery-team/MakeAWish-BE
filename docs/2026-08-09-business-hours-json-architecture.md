...
## 3. 사장님 앱 (MakeAWish-FE-Owner) 연관 변경 사항
- 기존 문자열 조합 함수를 폐기하고, 배열 객체를 `JSON.stringify()`를 통해 JSON 문자열로 직렬화하여 백엔드로 전송합니다.
- 백엔드에서 내려온 JSON 문자열을 `JSON.parse()`로 역직렬화하여 다시 7일 단위 에디터 UI에 바인딩합니다.

## 4. 백엔드 (MakeAWish-BE) 핵심 변경 사항
